defmodule ObserverWeb.Tracer.Server do
  ...
  @impl true
  def init(_args) do
    Logger.debug("Initializing Tracing Server")
    {:ok, %Tracer{}}
  end
  ...
  def handle_call({:stop_tracing, rcv_session_id}, _from, %Tracer{
    ...
    Logger.debug("The Trace session_id: #{inspect(session_id)} was requested to stop.")
  ...
  def handle_call(
    ...
    Logger.debug("New Trace Session: #{session_id} functions: #{inspect(functions_by_node)}")
  ...
  def handle_info({:trace_session_timeout, rcv_session_id} = msg, %Tracer{
      ...
      when rcv_session_id == session_id do
    Logger.debug("The Trace session_id: #{inspect(session_id)} timed out")
  ...
  def handle_info({:stop_tracing, rcv_session_id} = msg, %Tracer{
      ...
      when rcv_session_id == session_id do
    Logger.debug("Max messages (#{max_messages}) reached for session: #{inspect(session_id)}.")
