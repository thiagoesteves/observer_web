defmodule ObserverWeb.Tracer.Tool do
  ...
  @doc """
  Human-readable label for a traced process.

  Delegates to `ObserverWeb.ProcessLabel.resolve/1` - see that module for the fallback chain.
  Most traced processes on a remote node aren't locally registered (`Registry`/`:via`
  registrations don't set `:registered_name`), so those fallbacks are what keep multi-node
  reports readable.
  """
  @spec process_label(pid()) :: String.t()
  defdelegate process_label(pid), to: ObserverWeb.ProcessLabel, as: :resolve
