defmodule ObserverWeb.TelemetryTest do
  use ExUnit.Case, async: false

  import Mock
  import Mox

  alias ObserverWeb.Telemetry.Consumer, as: Consumer
  ...
  test "[un]subscribe_for_new_keys/0", %{node: node} do
    Consumer.subscribe_for_new_keys()
  ...
  test "[un]subscribe_for_new_data/0", %{node: node} do
    Consumer.subscribe_for_new_data(node, "vm.memory.total")
    ...
    Consumer.unsubscribe_for_new_data(node, "vm.memory.total")
  end
  ...
  test "get_keys_by_node/1 valid node", %{node: node} do
    ...
    assert ["vm.memory.total"] == Consumer.get_keys_by_node(node)
  end

  test "node/1 invalid node" do
    assert [] == Consumer.get_keys_by_node(nil)
  end

  test "list_data_by_node_key/3", %{node: node} do
    key_name = "test.phoenix"

    Consumer.subscribe_for_new_data(node, key_name)
    ...
    Enum.each(1..5, &Consumer.push_data(build_metric(node, key_name, &1)))
    ...
    assert [
           ...
           ] = Consumer.list_data_by_node_key(node |> to_string(), key_name, order: :asc)
    ...
    assert [
             ...
             %ObserverWeb.Telemetry.Data{timestamp: _, unit: _, value: 2, tags: _},
             %ObserverWeb.Telemetry.Data{timestamp: _, unit: _, value: 1, tags: _}
           ] = Consumer.list_data_by_node_key(node |> to_string(), key_name, order: :desc)
    ...
    assert [
             ...
             %ObserverWeb.Telemetry.Data{timestamp: _, unit: _, value: 4, tags: _},
             %ObserverWeb.Telemetry.Data{timestamp: _, unit: _, value: 5, tags: _}
           ] = Consumer.list_data_by_node_key(node |> to_string(), key_name)
  end
  ...
  test "Pruning expiring entries", %{node: node, pid: pid} do
    ...
    assert [] = Consumer.list_data_by_node_key(node |> to_string(), key_name, order: :asc)
  end
